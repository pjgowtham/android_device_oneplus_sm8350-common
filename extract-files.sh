...
function blob_fixup() {
    case "${1}" in
        ...
        odm/etc/camera/CameraHWConfiguration.config)
            ...
            ;;
        odm/lib/liblvimfs_wrapper.so|odm/lib64/libCOppLceTonemapAPI.so|odm/lib64/libSuperRaw.so|odm/lib64/libYTCommon.so|odm/lib64/libaps_frame_registration.so|odm/lib64/libyuv2.so|vendor/lib64/libalsc.so)
            [ "$2" = "" ] && return 0
            "${PATCHELF}" --replace-needed "libstdc++.so" "libstdc++_vendor.so" "${2}"
            ;;
        odm/lib64/libAlgoProcess.so|vendor/lib/libui-oplus.so)
            [ "$2" = "" ] && return 0
        ...
        vendor/lib/libgui1_vendor.so)
            [ "$2" = "" ] && return 0
            "${PATCHELF}" --replace-needed "libui.so" "libui-oplus.so" "${2}"
            ;;
